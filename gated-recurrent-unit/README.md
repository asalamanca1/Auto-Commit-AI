...
> If you're running **inference** or **evaluation**, make sure to:
> - Load the cleaned dataset (`cleaned_python_commit_dataset.csv`) to the `data/` folder
> - Load the pre-trained GRU weights (`gru_model.pt`) to the `trained_model/` folder
> - Have the custom BPE tokenizer (`custom_bpe_tokenizer.json`) in the project root(automatically loaded in the notebooks)

> If you're training from scratch, only the dataset and tokenizer are needed — skip loading the weights.
